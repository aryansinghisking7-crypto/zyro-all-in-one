...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isButton() && interaction.customId === 'create_ticket') {
      ...
      const ticket = await interaction.guild.channels.create({name: `ticket-${interaction.user.username}`,type: ChannelType.GuildText,parent: interaction.channel.parentId,permissionOverwrites: [{ id: interaction.guild.id, deny: [PermissionFlagsBits.ViewChannel] },{ id: interaction.user.id, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.AttachFiles] },{ id: SUPPORT_ROLE, allow: [PermissionFlagsBits.ViewChannel, PermissionFlagsBits.SendMessages, PermissionFlagsBits.ManageMessages] }]});
      claimedTickets.delete(ticket.id);
      const embed = new EmbedBuilder().setDescription(`Hello <@${interaction.user.id}>,\nA staff member will be with you shortly.\nPlease describe your issue in detail.`).setFooter({ text: 'Powered by ZYRO', iconURL: client.user.displayAvatarURL() }).setColor(0x2B2D31);
      const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('close_ticket').setLabel('Close').setStyle(ButtonStyle.Danger).setEmoji('🔒'),new ButtonBuilder().setCustomId('close_reason').setLabel('Close With Reason').setStyle(ButtonStyle.Danger).setEmoji('📝'),new ButtonBuilder().setCustomId('unclaim_btn').setLabel('Unclaim').setStyle(ButtonStyle.Secondary).setEmoji('🙋'));
    ...
    if (interaction.isButton() && interaction.customId === 'unclaim_btn') {if (claimedTickets.get(interaction.channel.id)!== interaction.user.id) return await interaction.reply({content: '❌ You did not claim this ticket', ephemeral: true});claimedTickets.delete(interaction.channel.id);await interaction.reply(`🙋 Ticket unclaimed by ${interaction.user}`);}
